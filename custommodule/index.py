...
def _j(level, u, k, w, data1, data2, center):
    d = _distance_c_x(level, u.shape[0], k, w, data1, data2, center)
    return ((u ** 2) * (d ** 2)).sum()
...
def pcaes(level, u, k, w, data1, data2):
    ...
    d = _distance_c_c(level, u.shape[0], k, w, data1, data2, center)
...
def xb(level, u, k, w, data1, data2):
    ...
    cd = _distance_c_c(level, u.shape[0], k, w, data1, data2, center)
    cd[cd == 0] = float('inf')
